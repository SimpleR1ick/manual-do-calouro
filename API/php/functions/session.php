...
function exibirLogin(): void {
    ...
    if (isset($_SESSION['id_usuario'])) {
        echo "<button class='btn btn-info text-white mt-2 mt-lg-0' type='button' onclick='window.location.href = \"../php/includes/logout.php\"'>Sair<i class=\"fa-solid fa-right-from-bracket ms-2\"></i></button>";
    // Se não existir um usuário, cria um botão para dar login
    } else {
        echo "<button class='btn btn-primary mt-2 mt-lg-0' type='button' onclick='window.location.href = \"login.php\"'>Entrar<i class=\"fa-solid fa-right-to-bracket ms-2\"></i></button>";
    }
}
